...
int tcpm(const char *src, const char *dst) {
  ...
  if ((ddf = open(ndst, O_RDWR | O_CREAT | O_TRUNC, per.st_mode)) <
      0) /*- change O_RD_ONLY to O_RDWR -*/
    sys_err("Fail to open target_file");

  /*- mmap need to pre-sizing dst size, otherwise cause bus error  40mins costs
   * there -*/
  ...
  if ((mr = mmap(0, per.st_size, PROT_READ, MAP_PRIVATE, sdf, 0)) ==
      MAP_FAILED) {
    sys_err("source file mmap err");
  }
  if ((mw = mmap(0, per.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, ddf, 0)) ==
      MAP_FAILED) {
