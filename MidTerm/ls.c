...
void build(int argc, char **argv) {
  ...
  while ((p = fts_read(ftsp)) != NULL) {
    switch (p->fts_info) {
    case FTS_DC:
      (void)fprintf(stderr, "%s: Directory cycle detected %s.", p->fts_name,
                    strerror(errno));
      exit(1);
      break;
    case FTS_ERR:
    case FTS_DNR:
      (void)fprintf(stderr, "%s: FTS directory error %s.", p->fts_name,
                    strerror(p->fts_errno));
      break;
    case FTS_D:
      /*- -a and -A option -*/
      if (p->fts_level != FTS_ROOTLEVEL && p->fts_name[0] == '.' && !f_lsdot) {
...
static void usage() { printf("usage: ls [AacdFfhiklnqRrSstuwl] [file ...]\n"); }

int compare(const FTSENT **a, const FTSENT **b) {
  int a_info, b_info;
  a_info = (*a)->fts_info;
  b_info = (*b)->fts_info;
  if (a_info == FTS_ERR || b_info == FTS_ERR || a_info == FTS_DNR ||
      b_info == FTS_DNR)
    return 0;
